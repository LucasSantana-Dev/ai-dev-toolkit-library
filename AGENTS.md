...
|---|---|---|---|
| `skill` | `~/.Codex/skills/<id>/` | `adtl install <id>` | Actionable procedure (multi-file) |
| `agent` | `~/.Codex/agents/<id>.md` | `adtl install <id>` | Codex sub-agent (single file) |
...
4. CLI install target: `~/.Codex/skills/<id>/`. Do not rewrite user's existing skill dirs — refuse with a clear error.
5. Docs kind (`catalog/docs/*.md`) has no install verb. If you're tempted to add one, it's a Skill.

## Non-obvious gotchas

- `mcp-context-forge` virtual-server UUID is pinned in `gateway/seeds/virtual-server.yaml`; changing it breaks everyone's `~/.Codex/mcpServers` config.
