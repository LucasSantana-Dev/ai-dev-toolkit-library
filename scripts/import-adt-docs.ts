 ...
 *
 * Sources:
 *   - ai-dev-toolkit/best-practices/*.md  → tag: best-practice
 *   - ai-dev-toolkit/companies/<slug>/README.md + COMPANY.md → tag: persona:<slug>
 *
...
async function importBestPractice(srcFile: string, destDir: string, written: string[], skipped: Array<[string, string]>) {
  ...
  const { content } = matter(raw); // strip any existing frontmatter from body
  ...
  const rebuilt = matter.stringify(body, {
    ...
    tags: tags.slice(0, 8),
    source: {
      path: `ai-dev-toolkit/best-practices/${name}.md`,
      upstream: `https://github.com/LucasSantana-Dev/ai-dev-toolkit/blob/main/best-practices/${name}.md`,
      license: "MIT",
    },
  });
...
async function importCompany(companyDir: string, destDir: string, written: string[], skipped: Array<[string, string]>) {
  ...
  const rebuilt = matter.stringify(body, {
    ...
    source: {
      path: `ai-dev-toolkit/companies/${companyName}`,
      upstream: `https://github.com/LucasSantana-Dev/ai-dev-toolkit/tree/main/companies/${companyName}`,
      license: "MIT",
    },
  });
  await writeFile(outFile, rebuilt);
...
async function main() {
  ...
  // Best practices
  console.log("▸ ai-dev-toolkit/best-practices");
  const bpDir = path.join(ADT_ROOT, "best-practices");
  if (existsSync(bpDir)) {
  ...
  // Companies / personas
  console.log("▸ ai-dev-toolkit/companies");
  const compDir = path.join(ADT_ROOT, "companies");
